...
import { checkBranchAvailability } from "@/app/(site)/dresses/actions";

/** A branch that holds the design free on the chosen date. */
...
export function AvailabilityCalendar({
  ...
  branchName: string;
  freeDates: string[];
  /** Where "View Similar Dresses" leads (the store wardrobe). */
  similarHref: string;
  /** WhatsApp number (normalised); null when not published. */
  ...
  const [result, setResult] = useState<
    | { available: true; date: string; branches: BranchAvailability[] }
    | { available: false }
    | null
  >(null);
  ...
  function check(event: React.FormEvent) {
    ...
    startTransition(async () => {
      // Re-read live, per branch: which branches hold this design free that date.
      const branches = await checkBranchAvailability(
        productSlug,
        productCode,
        productBranchSlug,
        chosen,
      ...
      if (branches && branches.length > 0) {
        setResult({ available: true, date: chosen, branches });
      } else {
        ...
        <div className="p-[clamp(1.15rem,4vw,1.75rem)]">
          {mode === "summary" ? (
            <>
              ...
              {/* Prominent highlighted result — the first thing to catch the eye. */}
              <div className="mt-[clamp(0.75rem,2vh,1rem)] border border-gold/45 bg-gold-soft/15 px-4 py-3.5">
                ...
                <ul className="mt-2.5 flex flex-wrap gap-2">
                  {branches.map((b) => (
                    <li
                      key={b.slug}
                      className="inline-flex items-center gap-1.5 border border-gold/40 bg-ivory px-3 py-1.5 font-sans text-[0.8125rem] font-medium text-ink"
                    >
                      <MapPin
                        aria-hidden="true"
